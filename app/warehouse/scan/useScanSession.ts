...
import { type AddResult } from "./AddItemModal";
import {
  beepBlocked,
  beepComplete,
  beepError,
...
export function hasAnyBarcode(it: { has_barcode?: boolean; barcode: string | null }): boolean {
...
export type FlashKind = "ok" | "error" | "complete" | "partial" | null;

export type WrongItemState = {
  itemName: string;
  message: string;
  pendingBarcode: string;
} | null;

export type OverQuantityState = {
  itemName: string;
  quantity: number;
  scannedCount: number;
  pendingBarcode: string;
...
export type InvoiceChangeState = {
  currentInvoice: {
    id: number;
    invoice_no: string;
    scanned_qty: number;
